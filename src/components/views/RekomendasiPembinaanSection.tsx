...
export const RekomendasiPembinaanSection: React.FC<RekomendasiPembinaanSectionProps> = ({
  ...
  const severity = getSeverityCategory(singlePoin);

  // Jika poin = 100 (Sangat Berat / Dikeluarkan)
  if (singlePoin >= 100) {
    return (
      <div className="mt-6 pt-6 border-t border-[#182740] animate-in fade-in duration-300">
        <div className="p-6 rounded-2xl bg-gradient-to-b from-[#180C14] to-[#12080E] border border-rose-500/50 shadow-2xl relative overflow-hidden space-y-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Ketentuan khusus pelanggaran tingkat tertinggi berdasarkan <strong className="text-rose-400">Poin Tunggal Pelanggaran</strong> ({singlePoin} Poin).
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <span className="px-3 py-1.5 rounded-lg bg-rose-500/15 border border-rose-500/30 text-rose-300 font-bold text-xs">
  ...
  if (!pembinaan) {
    return (
      <div className="mt-6 pt-6 border-t border-[#182740] animate-in fade-in duration-300">
        <div className="p-6 rounded-2xl bg-gradient-to-b from-[#0C182B] to-[#081120] border border-[#1E304F] shadow-2xl relative overflow-hidden space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Poin Tunggal Pelanggaran: <strong className="text-amber-400">{singlePoin} Poin</strong>
              </p>
            </div>
          </div>
  ...
  return (
    <div className="mt-6 pt-6 border-t border-[#182740] animate-in fade-in duration-300 space-y-5">
      {/* Outer Card Container */}
      <div className="p-5 sm:p-7 rounded-2xl bg-gradient-to-b from-[#0C182B] to-[#081120] border border-emerald-500/35 shadow-2xl relative overflow-hidden space-y-6">
        ...
        {/* ================================================================ */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#172B48]">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 shadow-sm shadow-emerald-500/20 shrink-0">
              <ShieldCheck className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-base font-extrabold text-white tracking-wide uppercase flex items-center gap-2 flex-wrap">
                <span>REKOMENDASI PEMBINAAN RESMI YAYASAN</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Ketentuan pembinaan otomatis berdasarkan <strong className="text-emerald-400">Poin Tunggal Pelanggaran</strong> ({singlePoin} Poin).
              </p>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
            <span className="px-3 py-1.5 rounded-lg bg-[#0E2038] border border-emerald-500/30 text-emerald-300 font-bold text-xs tracking-wide">
              {singlePoin} Poin Tunggal
            </span>
            <span className="px-3.5 py-1.5 rounded-lg bg-emerald-500 text-slate-950 font-black text-xs tracking-wider uppercase shadow-md shadow-emerald-500/30">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs bg-[#070F1C]/70 px-4 py-2.5 rounded-xl border border-[#172B48]">
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-300">
              Rentang Poin {pembinaan.nama_tingkat}:
            </span>
            <span className="px-2.5 py-0.5 rounded-md bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 font-extrabold text-xs">
        ...
        {/* ================================================================ */}
        <div className="p-4 sm:p-5 rounded-xl bg-gradient-to-r from-[#0E2038] to-[#0B1A2E] border border-teal-500/30 shadow-lg relative overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start gap-3.5">
              <div className="p-2.5 rounded-xl bg-teal-500/20 border border-teal-500/40 text-teal-300 shrink-0 mt-0.5">
              ...
              <div className="space-y-1">
                ...
                <div className="text-sm sm:text-base font-extrabold text-white flex items-center gap-2 flex-wrap">
                  <span>Target penyelesaian: <span className="text-emerald-400 font-black">{duration.targetHari} hari</span></span>
                ...
                <p className="text-[11px] text-slate-400">
                  Total masa pembinaan maksimal hingga <strong>{duration.totalHari} hari kalender</strong> sejak tanggal penetapan.
                </p>
              </div>
            </div>

            {/* Dynamic Date Range Display */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px] bg-[#070E1A]/80 p-2.5 rounded-lg border border-[#1A3152] shrink-0">
              <div>
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Tanggal Mulai</span>
                <span className="text-white font-semibold">{dates.formattedStartDate}</span>
              </div>
              <div>
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Target Selesai</span>
                <span className="text-emerald-400 font-semibold">{dates.formattedTargetEndDate}</span>
              </div>
              <div className="col-span-2 sm:col-span-1">
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Batas Toleransi</span>
                <span className="text-teal-300 font-semibold">{dates.formattedGraceEndDate}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* ================================================================ */}
        <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-[#070F1C]/90 p-4 rounded-xl border border-[#172B48]">
          <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-extrabold text-xs shadow-lg shadow-emerald-500/25 transition-all transform active:scale-95 cursor-pointer"
          >
...
const CategoryCard: React.FC<CategoryCardProps> = ({
  ...
  return (
    <div className="p-4 rounded-xl bg-[#081222] border border-[#192C4B] hover:border-emerald-500/35 transition-all flex flex-col justify-between space-y-3 group">
      <div className="flex items-center justify-between pb-2.5 border-b border-[#14233D]">
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded-md bg-[#0F2038] border border-[#1E365E] text-white font-bold text-[11px] flex items-center justify-center shrink-0">
            {number}
          </div>
          <h5 className="text-xs font-bold text-white tracking-wide uppercase truncate">
            {title}
          </h5>
        </div>
        <div className="p-1 rounded-md bg-[#0E1F36] shrink-0">
          {icon}
        </div>
      </div>
      ...
      <div className="space-y-2 text-xs flex-1">
        {items.length === 0 ? (
          <p className="text-[11px] italic text-slate-500 py-1">
            - Tidak ada butir khusus pada level ini -
          </p>
        ) : (
          items.map((item, idx) => (
            <div key={idx} className="flex items-start gap-2 text-slate-300 leading-relaxed text-[11.5px]">
      ...
      <div className="pt-2 border-t border-[#121E33] flex items-center justify-between text-[10px] text-slate-400">
  ...
  const categories = [
    {
      ...
      activeColor: 'bg-emerald-500 text-slate-950 font-black border-emerald-400 ring-2 ring-emerald-500/40',
    ...
    },
    {
      ...
      activeColor: 'bg-blue-500 text-white font-black border-blue-400 ring-2 ring-blue-500/40',
    ...
    },
    {
      ...
      activeColor: 'bg-amber-500 text-slate-950 font-black border-amber-400 ring-2 ring-amber-500/40',
    ...
    },
    {
      ...
      activeColor: 'bg-orange-500 text-white font-black border-orange-400 ring-2 ring-orange-500/40',
    ...
    },
    {
      ...
      activeColor: 'bg-rose-500 text-white font-black border-rose-400 ring-2 ring-rose-500/40',
      isMatch: currentPoin >= 100
    }
  ];

  return (
    <div className="pt-2 border-t border-[#172B48] space-y-2">
